...
results = struct();
 ...
 % Initialize cell arrays and arrays to store data for the table
    mcsIndices = {};
    distancesArray = [];
    snrValues = [];
    
    mcsCounter = 1; % Counter for MCS index
       ...
       %% Demodulate and Calculate BER
        ...
        for disIdx = 1:numDistances
            ...
            % Store data for table
                    for rxIdx = 1:numRx
                        mcsIndices{mcsCounter} = sprintf('%s_M%d_R%.0f', modScheme, M, codingRate * 100);
                        distancesArray(mcsCounter) = distances(disIdx);
                        snrValues(mcsCounter) = average_snr_dB(rxIdx);
        ...
        subplot(length(modulationSchemes) * length(codingRates), length(orders), ...
                    (schemeIdx-1) * length(orders) * length(codingRates) + (orderIdx-1) * length(codingRates) + rateIdx);
                plot(distances, snr_values, 'o', interpDistances, interpSNR, '-'); 
                title([modScheme ' M' num2str(M) ' Rate ' num2str(codingRate)]);
                xlabel('Distance (m)');
                ylabel('SNR (dB)');
                legend('Without RIS');
                grid on;
...
% Create and Display Table
    mcsTable = table(mcsIndices', distancesArray', snrValues', 'VariableNames', {'MCS_Index', 'Distance', 'SNR'});
    disp(mcsTable);

    % Save the table to a CSV file
    writetable(mcsTable, 'MCS_SNR_Distance_Table.csv');
